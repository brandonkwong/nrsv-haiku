...
function constructText (verse) {
  ...
  if (syllable(text) < MIN_SYLLABLES) {
    ...
    if (!nextVerse) throw new Error('Text does contain minimum required syllables.');
...
function reconstructText (text, linesOfWords) {
  ...
  function constructLine (words) {
    ...
    const lineTextIndexStart = words.length > 1
      ? textRef.indexOf(lineText)
      : textRef.lastIndexOf(lineText);
  ...
  for (let i = 0; i < linesOfWords.length; ++i) {
    const line = constructLine(linesOfWords[i]);
    lines.push(line);
  }
...
async function getLines (verse, options = {}) {
  ...
  // TODO: Retry with text slice of first word character
  if (isInvalid) {
    if (canRandomizeVerse(options)) {
      const randomVerse = await getVerse(options);
      return getLines(randomVerse, options);
    }

    // TODO: Display text with error
